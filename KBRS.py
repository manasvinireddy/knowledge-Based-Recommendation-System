...
def generateModel():
    ...
    for i in range(len(train)):
        ...
        if str(icon) != 'nan':
            # Use .get() to avoid KeyError if the emoji is not in UNICODE_EMOJI
            icon = UNICODE_EMOJI.get(icon, '')  # Default to an empty string if not found
        ...
        X.append(textdata)
        # Y.append(int(sentiment))  # If you need this later, uncomment this line
        count = count + len(arr)
    
    X = np.asarray(X)
    # Y = np.asarray(Y)  # If you need this later, uncomment this line
    # Y.clear()
...
def predict():
    ...
    for i in range(len(test)):
        email = test.iloc[i, 0]  # Updated to iloc
        tweet = test.iloc[i, 1]  # Updated to iloc
        arr = tweet.split(" ")
        icon = ''
        msg = ''
        ...
        for j in range(len(arr)):
            for emoji in UNICODE_EMOJI:
                if emoji == arr[j] and emoji in UNICODE_EMOJI:
                    icon = UNICODE_EMOJI[emoji]
                    
                    # Debugging: Check the type of icon
                    print(f"UNICODE_EMOJI[emoji]: {icon}, type: {type(icon)}")
                    
                    # If it's a dictionary, extract the string (adjust based on actual structure)
                    if isinstance(icon, dict):
                        icon = icon.get('emoji', '')  # Assuming 'emoji' is the key in the dictionary
                    
                    if isinstance(icon, str):  # Ensure icon is a string before calling re.sub
                        icon = ''.join(re.sub('[^A-Za-z\s]+', '', icon))
        ...
        if len(icon) > 0:
            for k in range(len(arr)-1):
                ...
                if len(word) > 2 and word not in stop_words:
                    msg += arr[k] + " "
            msg += icon  # Concatenate only if icon is a string
        else:
            for k in range(len(arr)):
                word = arr[k].strip()
                if len(word) > 2 and word not in stop_words:
